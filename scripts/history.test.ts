// Daily-series contract: missing days are 0, dates ascend and stay unique across DST shifts,
// level thresholds are 30/60/90/120 XP. This is what feeds the chart and the heat map.
import { describe, expect, test } from "bun:test";
import { calendarRows, dailySeries, levelFor, movingAverage } from "../src/lib/history";

describe("levelFor", () => {
  test("thresholds are 30/60/90/120 XP", () => {
...
describe("dailySeries", () => {
  test("ends today, missing days are 0, dates ascend", () => {
  ...
  test("dates stay unique across 370 days spanning a DST change", () => {
...
describe("movingAverage", () => {
  test("returns the right average even when the window is short at the start", () => {
    expect(movingAverage([10, 20, 30], 2)).toEqual([10, 15, 25]);
  });
});

describe("calendarRows", () => {
  test("count is that day's XP, level runs 0-4", () => {
