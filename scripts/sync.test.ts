...
describe("mergeProgress", () => {
  ...
  test("sayaçlar: XP en büyük, günler birleşim, seri yeni tarafın", () => {
    const local = base({ xp: 120, streak: 3, daysPlayed: ["2026-09-01"], lastDay: "2026-09-01", updatedAt: 5 });
    const remote = base({ xp: 40, streak: 7, daysPlayed: ["2026-09-02"], lastDay: "2026-09-02", updatedAt: 9 });
    const merged = mergeProgress(local, remote);
...
describe("normalizeProgress", () => {
  test("bozuk/eksik kayıt reddedilir", () => {
    ...
    expect(normalizeProgress({ version: 2, xp: 1, streak: 0, cards: {}, daysPlayed: [] })).toBeNull();
    expect(normalizeProgress("ilerleme")).toBeNull();
  });
  test("eksik alanlar tamamlanır", () => {
    const normalized = normalizeProgress({ version: 1, xp: 10, streak: 2, cards: {}, daysPlayed: [] });
